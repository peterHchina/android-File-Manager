...
public class CompressFilesUtil
{
	 ...
	 *             当压缩过程出错时抛出
	 */
	public static void zipFiles(File resFile, File zipFile) throws IOException
	{
		ZipOutputStream zipout = new ZipOutputStream(new BufferedOutputStream(new FileOutputStream(zipFile), BUFF_SIZE));
		zipout.setEncoding("GBK");
		zipFile(resFile, zipout, "");
	...
	public static void upZipFile(File zipFile, String folderPath) throws ZipException, IOException
	{
		...
		ZipFile zf = new ZipFile(zipFile.getName(), "GBK");
...
//		ZipFile zf = new ZipFile(zipFile);
	 ...
	 *             当压缩过程出错时抛出
	 */
	private static void zipFile(String FilePath) throws FileNotFoundException, IOException
	{

		String rootpath = FilePath;
		rootpath = new String(rootpath.getBytes("8859_1"), "GB2312");
		File resFile = new File(rootpath);
		ZipOutputStream zipout;
		if (resFile.isDirectory())
		{
			zipout = new ZipOutputStream(new FileOutputStream(resFile.getPath() + ".zip"));
			zipout.setEncoding("GBK");
		}
		else
		{
			if (resFile.getPath().lastIndexOf(".") == -1 || resFile.getPath().lastIndexOf(".") == resFile.getPath().length() - 1)
			{
				zipout = new ZipOutputStream(new FileOutputStream(resFile.getPath() + ".zip"));
			}
			zipout = new ZipOutputStream(new FileOutputStream(resFile.getPath().substring(0, resFile.getPath().lastIndexOf(".")) + ".zip"));
		}
